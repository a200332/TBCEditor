...
type
  ...
  type
    ...
    public
      function GetToken(ACurrentRule: TRange; APLine: PChar; var ARun: Integer; var AToken: TToken): Boolean;
        virtual; abstract;
    end;
    ...
    public
      ...
      destructor Destroy; override;
      function GetToken(ACurrentRange: TRange; APLine: PChar; var ARun: Integer; var AToken: TToken): Boolean; override;
...
function TBCEditorHighlighter.TParser.GetToken(ACurrentRange: TRange; APLine: PChar; var ARun: Integer;
...
function TBCEditorHighlighter.TDefaultParser.GetToken(ACurrentRange: TRange; APLine: PChar; var ARun: Integer;
  var AToken: TToken): Boolean;
begin
  Inc(ARun);
  Result := False;
...
function TBCEditorHighlighter.TDelimitersParser.GetToken(ACurrentRange: TRange; APLine: PChar; var ARun: Integer; var AToken: TToken): Boolean;
begin
  if APLine[ARun] <> BCEDITOR_NONE_CHAR then
    Inc(ARun);
  AToken := Self.Token;
...
begin
  ...
  begin
    ...
    if Assigned(FCurrentToken) and Assigned(FCurrentToken.OpenRule) then
      if FCurrentToken.OpenRule is TRange then
      begin
...
begin
  ...
  try
    if Editor.Visible then
      LCaretPosition := TextPosition(TCustomBCEditor(Editor).CaretPos.X + 1, TCustomBCEditor(Editor).CaretPos.Y);
    LTopLine := TCustomBCEditor(Editor).TopLine;
    LTempLines.AddStrings(TCustomBCEditor(Editor).Lines);
    TCustomBCEditor(Editor).Lines.Clear;
    ...
    TCustomBCEditor(Editor).Lines.Text := LTempLines.Text;
    TCustomBCEditor(Editor).TopLine := LTopLine;
    if Editor.Visible then
      TCustomBCEditor(Editor).CaretPos := Point(LCaretPosition.Char - 1, LCaretPosition.Line);
  finally
    LTempLines.Free;
  end;
  UpdateColors;
